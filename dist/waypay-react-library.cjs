...
`,Xo=({name:e,value:n,onChange:s,onBlur:i,onKeyPress:r,placeholder:c="8 (999) 500-50-50",customPlaceholder:p,tabIndex:g,disabled:j=!1,hasError:f=!1,errorMessage:m})=>{const[x,y]=w.useState(!1),d=w.useRef(null),h=!!(p&&!n&&!x),l=L=>{const A=L.replace(/\D/g,"").slice(0,11);let R="";return A.length>0&&(R+="8"),A.length>1&&(R+=` (${A.slice(1,4)}`),A.length>4&&(R+=`) ${A.slice(4,7)}`),A.length>7&&(R+=`-${A.slice(7,9)}`),A.length>9&&(R+=`-${A.slice(9,11)}`),R},I=L=>{const F=l(L.target.value),A={...L,target:{...L.target,name:e,value:F}};s(A)},k=L=>{y(!1),i?.(L)},E=L=>{r?.(L)};return t.jsxs(qo,{children:[t.jsx(Go,{ref:d,name:e,type:"tel",value:n,onChange:I,onKeyPress:E,placeholder:h?"":c,maxLength:18,tabIndex:g,disabled:j,hasError:f,onFocus:()=>y(!0),onBlur:k,inputMode:"tel"}),p&&t.jsx(Jo,{isVisible:h,children:p}),f&&m&&t.jsx(Ko,{children:m})]})},qo=o.div`
  position: relative;
  display: inline-block;
  width: 100%;
  margin-right: auto;
...
`,In=["zoomControl","fullscreenControl"];let ne=null;const Rn=e=>typeof window>"u"?Promise.reject(new Error("Window is not available")):window.ymaps?Promise.resolve(window.ymaps):(ne||(ne=new Promise((n,s)=>{const i=document.createElement("script"),r=e?`&apikey=${e}`:"";i.src=`https://api-maps.yandex.ru/2.1/?lang=ru_RU${r}`,i.async=!0,i.onload=()=>{window.ymaps?window.ymaps.ready(()=>n(window.ymaps)):s(new Error("Yandex Maps failed to load"))},i.onerror=()=>s(new Error("Failed to load Yandex Maps script")),document.head.appendChild(i)})),ne),Bn=({onLocationSelect:e,initialAddress:n="",initialLat:s=55.751244,initialLon:i=37.618423,apiKey:r,zoom:c=12,height:p=400,width:g="100%",inputPlaceholder:j="Введите адрес или название места",searchButtonLabel:f="Найти",draggablePlacemark:m=!0,controls:x=In,showSearch:y=!0,className:d,style:h,onError:l})=>{const[I,k]=w.useState(null),[E,L]=w.useState(n),[F,A]=w.useState(n),[R,Y]=w.useState(!1),B=w.useRef(null);w.useEffect(()=>{let _=!1,v;return(async()=>{try{const M=await Rn(r);if(_||!B.current)return;v=new M.Map(B.current,{center:[s,i],zoom:c,controls:x});const D=new M.Placemark([s,i],{},{draggable:m});v.geoObjects.add(D),v.events.add("click",N=>{const P=N.get("coords");z(P,D,v)}),D.events.add("dragend",()=>{const N=D.geometry.getCoordinates();z(N,D,v)}),k(v),Y(!0)}catch(M){l&&M instanceof Error&&l(M)}})(),()=>{_=!0,v&&v.destroy()}},[r,s,i,c,x,m,l]);const z=async(_,v,u)=>{if(window.ymaps)try{const N=(await window.ymaps.geocode(_)).geoObjects.get(0).getAddressLine();v.geometry.setCoordinates(_),u.panTo(_,{flying:!0}),A(N),L(N),e(N,_[0],_[1])}catch(M){l&&M instanceof Error&&l(M)}},W=async()=>{if(!(!E.trim()||!window.ymaps||!I))try{const v=(await window.ymaps.geocode(E)).geoObjects.get(0);if(v){const u=v.geometry.getCoordinates(),M=v.properties.get("boundedBy");I.setBounds(M,{checkZoomRange:!0});const D=new window.ymaps.Placemark(u,{},{draggable:m});I.geoObjects.removeAll(),I.geoObjects.add(D);const N=v.getAddressLine();A(N),e(N,u[0],u[1]),D.events.add("dragend",()=>{const P=D.geometry.getCoordinates();z(P,D,I)})}}catch(_){l&&_ instanceof Error&&l(_)}};return t.jsxs(An,{className:d,style:h,children:[y&&t.jsxs(zn,{children:[t.jsx(le,{type:"text",value:E,onChange:_=>L(_.target.value),placeholder:j,onKeyPress:_=>_.key==="Enter"&&W(),disabled:!R}),t.jsx(Fn,{onClick:W,disabled:!R,children:f})]}),t.jsxs(On,{children:["Выбранный адрес: ",t.jsx("strong",{children:F||"Не выбран"})]}),t.jsx(_n,{ref:B,$height:p,$width:g}),t.jsx(Nn,{children:"📍 Кликните на карте или найдите адрес через поиск"})]})},An=o.div`
  display: flex;
  flex-direction: column;
  gap: 12px;
`,zn=o.div`
...
`,Pe=({isOpen:e,callPhone:n,error:s,onClose:i})=>{const[r,c]=w.useState(!1);if(!e)return null;const p=/iPhone|iPad|iPod|Android/i.test(navigator.userAgent),g=()=>{p?window.location.href=`tel:${n}`:(navigator.clipboard.writeText(n),c(!0),setTimeout(()=>c(!1),2e3))};return t.jsx(Or,{onClick:i,children:t.jsxs(Nr,{onClick:j=>j.stopPropagation(),children:[t.jsx(Yr,{children:"Подтверждение авторизации"}),t.jsx(Se,{children:"Для завершения авторизации позвоните на номер:"}),t.jsxs(Wr,{onClick:g,title:p?"Позвонить":"Копировать номер",$clickable:!0,children:[n,!p&&r&&t.jsx(Hr,{children:"Скопировано!"})]}),t.jsx(Se,{children:"После звонка система автоматически подтвердит вашу авторизацию."}),s&&t.jsx(Vr,{children:s})]})})},Or=o.div`
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
...
`,Vr=o.div`
  color: #e74c3c;
  margin: 10px 0;
`,Ie=w.createContext({collapsed:!1,toggleCollapsed:()=>{},selectedSideMenuItem:null,setSelectedSideMenuItem:()=>{},selectedTopMenuItem:null,setSelectedTopMenuItem:()=>{}}),Zr=({items:e,bottomItems:n=[],logo:s,arrowIcon:i,collapsed:r,onToggle:c,selectedKey:p,onSelect:g,autoCollapseOnSelect:j=!1,width:f="315px",collapsedWidth:m="70px",className:x,style:y})=>{const d=w.useContext(Ie),[h,l]=w.useState(r??d.collapsed??!1),[I,k]=w.useState(null),[E,L]=w.useState(p??d.selectedSideMenuItem??null),[F,A]=w.useState({visible:!1,x:0,y:0,items:[]}),R=w.useRef(null),Y=r??d.collapsed??h,B=P=>{l(P),d.toggleCollapsed(),c?.(P)},z=p??d.selectedSideMenuItem??E,W=P=>{L(P),d.setSelectedSideMenuItem(P)};w.useEffect(()=>{r!==void 0&&(l(r),r&&k(null))},[r]),w.useEffect(()=>{p!==void 0&&W(p)},[p]),w.useEffect(()=>{const P=()=>A(V=>({...V,visible:!1}));return document.addEventListener("click",P),()=>document.removeEventListener("click",P)},[]);const _=()=>{const P=!Y;B(P),P&&k(null)},v=P=>{P.disabled||P.hidden||(W(P.key),g?.(P),P.onClick?.(P),j&&(B(!0),k(null)))},u=P=>{k(V=>V===P?null:P)},M=(P,V)=>{const b=(P.children?.length||0)>0;if(Y&&b){V.stopPropagation();const C=V.currentTarget.getBoundingClientRect();A({visible:!0,x:C.right+6,y:C.top,items:P.children||[]});return}b?u(P.key):v(P)},D=(P,V=!1)=>P.filter(b=>!b.hidden).map(b=>{const C=(b.children?.length||0)>0,a=I===b.key,$=z===b.key||!!b.children&&b.children.some(S=>S.key===z);return t.jsxs(w.Fragment,{children:[t.jsxs(Qr,{$collapsed:Y,$selected:$,onClick:S=>M(b,S),$disabled:b.disabled,title:b.label,children:[b.icon&&t.jsx(ti,{$collapsed:Y,children:b.icon}),!Y&&t.jsx(ei,{children:b.label}),!Y&&C&&t.jsx(ni,{$expanded:a,children:"▾"})]}),!Y&&C&&a&&t.jsx(oi,{children:D(b.children||[],!0)})]},b.key)}),N=w.useMemo(()=>F.visible?t.jsx(ii,{style:{left:F.x,top:F.y},children:F.items.map(P=>t.jsx(si,{onClick:()=>{v(P),A({visible:!1,x:0,y:0,items:[]})},$selected:z===P.key,children:P.label},P.key))}):null,[F,z]);return t.jsxs(Ur,{ref:R,$collapsed:Y,$width:f,$collapsedWidth:m,className:x,style:y,children:[t.jsxs(Xr,{children:[t.jsx(qr,{onClick:_,"aria-label":"Переключить меню",children:i??t.jsx(Gr,{$collapsed:Y,children:"❮"})}),!Y&&s&&t.jsx(Jr,{children:s})]}),t.jsxs(Kr,{children:[t.jsx(Me,{children:D(e)}),n.length>0&&t.jsxs(t.Fragment,{children:[t.jsx(ri,{}),t.jsx(Me,{children:D(n)})]})]}),N,t.jsx(ai,{$collapsed:Y})]})},Ur=o.div`
...
`,ji=o.div`
  display: flex;
  gap: 12px;
`,Ci=({onLoginSuccess:e,userType:n="delivery_operator",title:s="Авторизация",submitLabel:i="Войти",placeholder:r="8 (xxx) xxx-xx-xx",supportPhone:c="8 989 924 24 24",supportLinkUrl:p="https://waypay.one",supportLinkText:g="waypay.one",className:j,style:f,pollingIntervalMs:m=3e3,enableRegistration:x=!1})=>{const[y,d]=w.useState(!1),[h,l]=w.useState(""),[I,k]=w.useState(!1),[E,L]=w.useState(""),[F,A]=w.useState(""),[R,Y]=w.useState(!1),[B,z]=w.useState(!1),[W,_]=w.useState(""),[v,u]=w.useState(""),[M,D]=w.useState(null),N=w.useRef(void 0);w.useEffect(()=>()=>{N.current&&clearInterval(N.current)},[]);const P=S=>{const T=S.replace(/\D/g,"");return T?T.startsWith("8")?`+7${T.slice(1)}`:T.startsWith("7")?`+${T}`:`+${T}`:""},V=()=>{k(!1),z(!1),_(""),u(""),D(null),N.current&&window.clearInterval(N.current)},b=async S=>{if(S.preventDefault(),!h){A("Введите номер телефона");return}d(!0),A("");const T=P(h);try{const O=await Be(T,n);D({phone:T,checkId:O.check_id,callPhone:O.call_phone}),x&&"name_required"in O&&O.name_required?(z(!0),L(O.call_phone)):!x&&"name_required"in O&&O.name_required?(A("Этот номер не зарегистрирован в системе. Обратитесь в поддержку для регистрации."),D(null)):(L(O.call_phone),k(!0),a(T,O.check_id))}catch(O){console.error("Auth error:",O),A("Не удалось инициировать авторизацию. Проверьте номер.")}finally{d(!1)}},C=async S=>{if(!S.trim()){u("Введите ФИО");return}if(M){d(!0),u("");try{_(S),z(!1),k(!0),a(M.phone,M.checkId,S)}catch(T){console.error("Registration error:",T),u("Не удалось завершить регистрацию")}finally{d(!1)}}},a=(S,T,O)=>{N.current&&window.clearInterval(N.current),N.current=window.setInterval(()=>{$(S,T,O)},m)},$=async(S,T,O)=>{if(!R){Y(!0);try{const Z=await Ae(S,T,n,O||W);Z.access_token&&Z.refresh_token&&(N.current&&window.clearInterval(N.current),k(!1),z(!1),_(""),D(null),e(Z.access_token))}catch(Z){console.error("Verification error:",Z),A("Ошибка проверки авторизации")}finally{Y(!1)}}};return t.jsxs($i,{className:j,style:f,children:[t.jsxs(Si,{children:[t.jsxs(Mi,{onSubmit:b,children:[t.jsx(Ti,{children:s}),t.jsx(Di,{children:t.jsx(Li,{children:t.jsx(Re,{value:h,onChange:S=>l(S),placeholder:r})})}),F&&t.jsx(Bi,{children:t.jsx("span",{children:F})}),t.jsx(Ei,{type:"submit",disabled:y,children:y?"Загрузка...":i})]}),t.jsxs(Pi,{children:["Хотите получить доступ к нашим продуктам?",t.jsx("br",{}),"Обратитесь по номеру"," ",t.jsx(Ii,{href:`tel:${c}`,children:c}),t.jsx("br",{}),"или оставьте заявку на"," ",t.jsx(Ri,{href:p,target:"_blank",rel:"noreferrer",children:g})]})]}),t.jsx(pi,{isOpen:B,onClose:V,onSubmit:C,error:v,isLoading:y,callPhone:E}),t.jsx(Pe,{isOpen:I,onClose:V,callPhone:E,error:F})]})},$i=o.div`
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
...
`,ze=e=>{let n=0;for(let i=0;i<e.length;i++)n=e.charCodeAt(i)+((n<<5)-n);return`hsl(${Math.abs(n%360)}, 60%, 60%)`},Ai=e=>!e||!e.trim()?"?":e.split(" ").filter(i=>i.length>0).map(i=>i[0].toUpperCase()).join("").slice(0,2)||"?",zi=(e,n=100)=>{const s=document.createElement("canvas");s.width=n,s.height=n;const i=s.getContext("2d");if(!i)return"";const r=ze(e);i.fillStyle=r,i.beginPath(),i.arc(n/2,n/2,n/2,0,Math.PI*2),i.fill(),i.fillStyle="#ffffff";const c=n/2;i.font=`bold ${c}px "Manrope", Arial, sans-serif`,i.textAlign="center",i.textBaseline="middle";const p=c*.1;return i.fillText(e,n/2,n/2+p),s.toDataURL("image/png")},Fi=e=>{if(!e)return"";const n=typeof e=="string"?new Date(e):e;if(isNaN(n.getTime()))return"";const s=n.getDate().toString().padStart(2,"0"),i=(n.getMonth()+1).toString().padStart(2,"0"),r=n.getFullYear();return`${s}-${i}-${r}`};function _i(e){return new Date(e)}const Fe=e=>{let n;try{if(e instanceof Date)n=e;else if(typeof e=="string"||typeof e=="number")n=new Date(e);else throw new Error("Неподдерживаемый тип данных");if(isNaN(n.getTime()))return"неверная дата";const s=new Date;if(n>s){const m=Math.floor((n.getTime()-s.getTime())/1e3),x=Math.floor(m/60),y=Math.floor(x/60),d=Math.floor(y/24);return m<10?"скоро":m<60?`через ${m} секунд`:x===1?"через минуту":x<60?`через ${x} минут`:y===1?"через час":y<24?`через ${y} часов`:d===1?"завтра":d<7?`через ${d} дней`:"в будущем"}const i=Math.floor((s.getTime()-n.getTime())/1e3),r=Math.floor(i/60),c=Math.floor(r/60),p=Math.floor(c/24),g=Math.floor(p/7),j=Math.floor(p/30),f=Math.floor(p/365);if(i<10)return"только что";if(i<60){const m=Math.max(1,i);return m===1?"секунду назад":m<5?`${m} секунды назад`:`${m} секунд назад`}return r<60?r===1?"минуту назад":r<5?`${r} минуты назад`:`${r} минут назад`:c<24?c===1?"час назад":c<5?`${c} часа назад`:`${c} часов назад`:p<7?p===1?"вчера":p===2?"позавчера":`${p} дня назад`:g<4?g===1?"неделю назад":g<5?`${g} недели назад`:`${g} недель назад`:j<12?j===1?"месяц назад":j<5?`${j} месяца назад`:`${j} месяцев назад`:f===1?"год назад":f<5?`${f} года назад`:`${f} лет назад`}catch(s){return console.error("Ошибка форматирования относительного времени:",s),"ошибка даты"}},_e=e=>{try{return e.toLocaleDateString("ru-RU",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"})}catch{return"неверная дата"}},Oi=(e,n={})=>{const{useExactAfterMonths:s=6,includeTime:i=!0}=n,r=new Date(e);return isNaN(r.getTime()),Math.floor((new Date().getTime()-r.getTime())/(1e3*60*60*24*30))>=s?i?_e(r):r.toLocaleDateString("ru-RU",{day:"2-digit",month:"2-digit",year:"2-digit"}):Fe(r)};exports.API_BASE_URL=ae;exports.AddButton=lt;exports.AuthCallModal=Pe;exports.AuthPage=Ci;exports.BackButton=xt;exports.BaseModal=De;exports.Checkbox=Nt;exports.ColumnChart=Wn;exports.ContextMenu=se;exports.CountInfoTitle=bn;exports.CustomCalendar=re;exports.CustomInput=le;exports.CustomPhoneInput=Re;exports.CustomSelect=St;exports.CustomTable=Ge;exports.CustomTextArea=bo;exports.CustomTitle=de;exports.DefaultButton=Q;exports.DeleteButton=bt;exports.EditButton=wt;exports.EntitySelectContainer=an;exports.ExportButton=kt;exports.InputTitle=wn;exports.ModalInfoTitle=vn;exports.Pagination=fo;exports.PeriodField=Ro;exports.PhoneModalInput=Xo;exports.PhotoViewerModal=Lr;exports.ProgressBar=Tn;exports.RadioButton=$n;exports.ScatterPlot=pr;exports.SideMenu=Zr;exports.SideMenuContext=Ie;exports.Tabs=Qo;exports.TitleField=ce;exports.ToggleSwitch=Cn;exports.YandexMapSelector=Bn;exports.apiRequest=q;exports.authStorage=ut;exports.confirmCallAuth=Ae;exports.formatDate=Fi;exports.formatExactDate=_e;exports.formatMoney=ie;exports.formatRelativeTime=Fe;exports.formatRelativeTimeExtended=Oi;exports.generateAvatar=zi;exports.getInitials=Ai;exports.getSessionInfo=di;exports.initiateCallAuth=Be;exports.parseDateInput=_i;exports.parseMoney=Yn;exports.stringToColor=ze;
