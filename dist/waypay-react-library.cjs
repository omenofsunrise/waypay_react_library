...
`,Ti=n.div`
  ...
  margin-top: 8px;
  text-align: left;
`,Di=({onLoginSuccess:e,userType:o="delivery_operator",title:i="Авторизация",submitLabel:r="Войти",placeholder:s="8 (xxx) xxx-xx-xx",supportPhone:c="8 989 924 24 24",supportLinkUrl:u="https://waypay.one",supportLinkText:g="waypay.one",className:j,style:b,pollingIntervalMs:w=3e3,enableRegistration:x=!1})=>{const[y,h]=m.useState(!1),[p,l]=m.useState(""),[I,k]=m.useState(!1),[P,E]=m.useState(""),[O,A]=m.useState(""),[R,H]=m.useState(!1),[z,_]=m.useState(!1),[W,F]=m.useState(""),[v,d]=m.useState(""),[M,D]=m.useState(null),N=m.useRef(void 0);m.useEffect(()=>()=>{N.current&&clearInterval(N.current)},[]);const V=S=>{const f=S.replace(/\D/g,"");return f?f.startsWith("8")?`+7${f.slice(1)}`:f.startsWith("7")?`+${f}`:`+${f}`:""},Z=()=>{k(!1),_(!1),F(""),d(""),D(null),N.current&&window.clearInterval(N.current)},C=async S=>{if(S.preventDefault(),!p){A("Введите номер телефона");return}h(!0),A("");const f=V(p);try{const T=await Fe(f,o);D({phone:f,checkId:T.check_id,callPhone:T.call_phone}),x&&"name_required"in T&&T.name_required?(_(!0),E(T.call_phone)):!x&&"name_required"in T&&T.name_required?(A("Этот номер не зарегистрирован в системе. Обратитесь в поддержку для регистрации."),D(null)):(E(T.call_phone),k(!0),a(f,T.check_id))}catch(T){console.error("Auth error:",T),T&&T.response&&T.response.data?A(T.response.data):T instanceof Error?A(T.message):A("Произошла ошибка при авторизации")}finally{h(!1)}},L=async S=>{if(!S.trim()){d("Введите ФИО");return}if(M){h(!0),d("");try{F(S),_(!1),k(!0),a(M.phone,M.checkId,S)}catch(f){console.error("Registration error:",f),d("Не удалось завершить регистрацию")}finally{h(!1)}}},a=(S,f,T)=>{N.current&&window.clearInterval(N.current),N.current=window.setInterval(()=>{$(S,f,T)},w)},$=async(S,f,T)=>{if(!R){H(!0);try{const B=await _e(S,f,o,T||W);(B.access_token&&B.refresh_token||B.accessToken&&B.refreshToken)&&(N.current&&window.clearInterval(N.current),k(!1),_(!1),F(""),D(null),e(B.access_token??B.accessToken))}catch(B){console.error("Verification error:",B)}finally{H(!1)}}};return t.jsxs(Ei,{className:j,style:b,children:[t.jsxs(Ii,{children:[t.jsxs(Pi,{onSubmit:C,children:[t.jsx(Ri,{children:i}),t.jsx(Bi,{children:t.jsx(zi,{children:t.jsx(Ae,{value:p,onChange:S=>l(S),placeholder:s})})}),O&&t.jsx(Ni,{children:t.jsx("span",{children:O})}),t.jsx(Ai,{type:"submit",disabled:y,children:y?"Загрузка...":r})]}),t.jsxs(Fi,{children:["Хотите получить доступ к нашим продуктам?",t.jsx("br",{}),"Обратитесь по номеру"," ",t.jsx(_i,{href:`tel:${c}`,children:c}),t.jsx("br",{}),"или оставьте заявку на"," ",t.jsx(Oi,{href:u,target:"_blank",rel:"noreferrer",children:g})]})]}),t.jsx(ki,{isOpen:z,onClose:Z,onSubmit:L,error:v,isLoading:y}),t.jsx(Be,{isOpen:I,onClose:Z,callPhone:P,error:O})]})},Ei=n.div`
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
