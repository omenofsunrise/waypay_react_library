...
`,te="access_token",ht="refresh_token",J={setTokens(e){localStorage.setItem(te,e)},getAccessToken(){return localStorage.getItem(te)},clear(){localStorage.removeItem(te),localStorage.removeItem(ht)}},xt=o.button`
  color: black;
  display: flex;
  align-items: center;
  gap: 8px;
...
`,ce=o.h1`
  ...
  color: black;
  font-weight: 600;
`;function ie(e,n="₽"){if(!e||e.trim()==="")return`0${n}`;const s=e.replace(/[^\d-]/g,"");if(!s||s==="-"||s==="")return`0${n}`;const i=parseInt(s,10);if(isNaN(i))return`0${n}`;const r=new Intl.NumberFormat("ru-RU",{style:"decimal",minimumFractionDigits:0,maximumFractionDigits:0}).format(i);return!0?`${r}${n}`:r}function Nn(e){if(!e)return 0;const n=e.replace(/\D/g,"");return parseInt(n,10)}const Hn=({title:e,data:n,maxValue:s,step:i,height:r=300,titleFontSize:d})=>{const[p,m]=y.useState(null),j=[];for(let g=0;g<=s;g+=i)j.push(g);const f=n.length>0?n[0].bars.map(g=>({name:g.name,color:g.color})):[];return t.jsxs(Vn,{children:[t.jsxs(le,{style:{marginTop:"0"},children:[t.jsx(ce,{style:{fontSize:d},children:e}),t.jsx(Zn,{children:f.map((g,x)=>t.jsxs(Un,{children:[t.jsx(Xn,{color:g.color}),t.jsx(qn,{children:g.name})]},x))})]}),t.jsxs(Gn,{height:r,children:[t.jsx(Kn,{children:j.map((g,x)=>t.jsx(Jn,{children:g.toLocaleString()},x))}),t.jsxs(Qn,{children:[t.jsxs(er,{children:[j.map((g,x)=>{const b=g/s*100;return t.jsx(tr,{position:100-b},`line-${x}`)}),n.map((g,x)=>t.jsxs(or,{onMouseEnter:()=>m(x),onMouseLeave:()=>m(null),children:[g.bars.map((b,h)=>{const u=g.bars.slice(h+1).reduce((l,L)=>l+L.value,0);return t.jsx(nr,{height:b.value/s*100,offset:u/s*100,color:b.color,isHovered:p!==null&&p!==x},h)}),p===x&&t.jsxs(sr,{children:[t.jsxs(ar,{children:[t.jsxs(lr,{children:["Всего: ",ie(g.total.toString())]}),g.bars.map((b,h)=>t.jsxs(cr,{children:[b.name,": ",ie(b.value.toString())]},h))]}),t.jsx(dr,{})]})]},x))]}),t.jsx(rr,{children:n.map((g,x)=>t.jsx(ir,{style:{flex:1,textAlign:"center"},children:g.label},x))})]})]})]})},Vn=o.div`
  background-color: white;
  border-radius: 10px;
  border: 1px solid rgba(209, 213, 219, 1);
  padding: 20px;
